...
export default function Main1() {
  ...
  const [dropdownChatId, setDropdownChatId] = useState(null);  // dropdown state
        ...
        <div className="fixed inset-0 z-50 md:hidden bg-black bg-opacity-50" onClick={() => setIsSidebarOpen(false)}>
          <aside className="fixed left-0 top-0 h-full w-80 bg-gray-800 border-r border-gray-700 p-4 flex flex-col z-50">
            ...
            <nav className="flex-1 overflow-auto space-y-2">
              {chats.map(chat => (
                <div key={chat.id} className={`relative flex items-center justify-between px-2 py-2 rounded-md hover:bg-gray-700 cursor-pointer ${chat.id === activeChatId ? 'bg-gray-700' : ''}`}>
            ...
            <div className="flex items-center bg-gray-800/50 border border-gray-700 rounded-full px-2 md:px-3 py-1 md:py-2 flex-1 w-full">  
              <div className="flex items-center gap-1 md:gap-2 mr-2 md:mr-4">  
                ...
                <select value={model} onChange={e => setModel(e.target.value)}
                  className="text-xs md:text-sm bg-gray-800 border border-gray-700 px-2 md:px-3 py-1 rounded-full text-gray-100 outline-none cursor-pointer appearance-none"
                >
              ...
              <button type="submit" className="ml-1 md:ml-3 bg-blue-600 hover:bg-blue-500 rounded-full p-2 md:p-3 flex items-center justify-center">
